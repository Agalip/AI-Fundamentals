...
# Artificial Intelligence Fundamentals - Assignment 5 - Mamdami's
...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        x_diff = fuzzcontrol.Antecedent(np.arange(-200, 201, 1), 'x_diff')
        y_diff = fuzzcontrol.Antecedent(np.arange(-200, 201, 1), 'y_diff')

        # Define output variable
        output = fuzzcontrol.Consequent(np.arange(-50, 50, 1), 'output')

        # Define fuzzy sets for input variables
        x_diff['left'] = fuzz.trapmf(x_diff.universe, [-200, -200, -100, 0])
        x_diff['center'] = fuzz.trimf(x_diff.universe, [-100, 0, 100])
        x_diff['right'] = fuzz.trapmf(x_diff.universe, [0, 100, 200, 200])

        y_diff['far'] = fuzz.trapmf(y_diff.universe, [-200, -200, -100, 0])
        y_diff['close'] = fuzz.trimf(y_diff.universe, [-100, 0, 100])
        y_diff['over'] = fuzz.trapmf(y_diff.universe, [0, 100, 200, 200])

        # Define fuzzy sets for output variable
        output['right'] = fuzz.trimf(output.universe, [-50, -50, 0])
        output['center'] = fuzz.trimf(output.universe, [-30, 0, 30])
        output['left'] = fuzz.trimf(output.universe, [0, 50, 50])

        # Define the rules and set their weights
        rules = [
            fuzzcontrol.Rule(x_diff['left'] & y_diff['far'], output['left']),
            fuzzcontrol.Rule(x_diff['left'] & y_diff['close'], output['center']),
            fuzzcontrol.Rule(x_diff['left'] & y_diff['over'], output['left']),
            
            fuzzcontrol.Rule(x_diff['center'] & y_diff['far'], output['center']),
            fuzzcontrol.Rule(x_diff['center'] & y_diff['close'], output['center']),
            fuzzcontrol.Rule(x_diff['center'] & y_diff['over'], output['center']),
            
            fuzzcontrol.Rule(x_diff['right'] & y_diff['far'], output['right']),
            fuzzcontrol.Rule(x_diff['right'] & y_diff['close'], output['center']),
            fuzzcontrol.Rule(x_diff['right'] & y_diff['over'], output['right']),
        ]

        # Define the control system
        control_system = fuzzcontrol.ControlSystem(rules)
        self.controller = fuzzcontrol.ControlSystemSimulation(control_system)

    def make_decision(self, x_diff: int, y_diff: int) -> int:
        #Make the controller compute the next movement
        self.controller.input['x_diff'] = x_diff
        self.controller.input['y_diff'] = y_diff
        
        # Compute the output using the defined rules
        self.controller.compute()
        
        # Return the computed output
        return int(self.controller.output['output'])
