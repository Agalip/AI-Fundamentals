...
# Artificial Intelligence Fundamentals - Assignment 4
...
class SearchBasedPlayer(Player):
    ...
    def search_path(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        ...
        visited = set()
        # Get the starting position of the snake
        start = snake.positions[0]
        # Get the position of the food
        goal = food.position
        # Create a set of obstacle positions
        obstacle_positions = set()
        for o in obstacles[0]:
            obstacle_positions.add(o.position)

        # Define the heuristic function that estimates the distance from a position to the goal
        def heuristic(position: Position, path_cost: int) -> Tuple[int, int]:
            # We use the Manhattan distance as our heuristic 
            estimated_cost = abs(position.x - goal.x) + abs(position.y - goal.y)
            total_cost = path_cost + estimated_cost
            return (total_cost, position.x * GRID_WIDTH + position.y)

        # Initialize a priority queue (heap) with the starting position and an empty path
        queue = [(heuristic(start, 0), start, [], 0)]
        heapq.heapify(queue)

        # Keep searching as long as there are positions in the queue
        while queue:
            # Pop the position with the lowest heuristic value (priority) from the queue
            _, curr_pos, path, path_cost = heapq.heappop(queue)
            # If the current position is the goal, set the chosen_path attribute and return
            if curr_pos == goal:
            ...
            # Loop through all possible directions that the snake can move in
            for direction in Direction:
                # Calculate the position that the snake will move to if it moves in the current direction
                next_pos = Position(curr_pos.x + direction.value[0], curr_pos.y + direction.value[1])
                # If the next position is out of bounds, or it is an obstacle, or the snake has already visited that position, skip it
                if next_pos.check_bounds(GRID_WIDTH, GRID_HEIGHT) or next_pos in obstacle_positions or next_pos in snake.positions:
                    continue
                # Otherwise, add the next position to the queue along with the path that led to it and the total path cost
                heapq.heappush(queue, (heuristic(next_pos, path_cost + 1), next_pos, path + [direction], path_cost + 1))

"""
To make it a blind search algorithm, we would need to remove the heuristic function and the tiebreaker value 
from the priority queue. Blind search algorithms do not use any additional knowledge or heuristics to guide 
the search and instead explore all possible paths from the starting position. In this case, we would simply 
initialize the priority queue with the starting position and an empty path.

To use Dijkstra's algorithm, we would need to modify the heuristic function to use the actual distance from 
the starting position to the current position instead of the estimated distance to the goal. 
Dijkstra's algorithm is a shortest path algorithm that uses a priority queue to explore the nodes 
in order of their distance from the starting node. In this case, the priority queue would be initialized 
with the starting position and an empty path, and the heuristic function would be modified 
to calculate the distance from the starting position to the current position using the actual path cost so far.
